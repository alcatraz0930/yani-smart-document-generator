...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const structureRules = {
      'Business Letter': 'Use a conventional business-letter structure with an appropriate greeting and professional closing. Courtesy language such as thanking the recipient may be used when supported by the notes. If no document date is provided, include [Date]. If recipient name, sender name, or other critical identity details are missing, use clear square-bracket placeholders such as [Client Name] or [Your Name].',
      'Proposal': 'Use a concise professional proposal structure focused on the client need, proposed approach, scope or deliverables, expected benefits, and next steps only when supported by the notes. Do not write it like a thank-you letter or meeting follow-up unless the user explicitly asks for that style. If the proposal date or key party details are missing, use square-bracket placeholders.',
      'Meeting Summary': 'Use a factual, neutral meeting-summary structure with Date, Attendees, Topic/Purpose, Key Discussion Points, Decisions, Next Steps, and Action Items only when supported. Do not use greetings, thank-you language, conversational openings, sign-offs, or letter-style courtesy phrasing. Under Key Discussion Points, summarize only the substantive matters actually discussed. Under Decisions, Next Steps, and Action Items, state only items supported by the notes. If the meeting date or attendees are missing, use [Date] or [Attendees] placeholders rather than inventing them.',
      'Memo': 'Use a direct internal-business memo structure with Date, To, From, and Subject headers, followed by concise purpose, information, and required action or next steps when supported. Do not use greetings, thank-you language, conversational openings, complimentary closes, or sign-offs unless the user explicitly requests them. Convert any courtesy-style wording in the notes into neutral memo language. Use [Date], [Recipient], [Your Name], or [Subject] when required information is missing.',
